...
NINA is an AI coding agent that **runs in the terminal**. You tell it a goal — NINA plans, writes files, runs commands, and fixes errors. You just watch.
...
| 🔄 **Automatic Error Recovery** | Self-recovery loop for command/action failures |
| 📺 **Live Monitor** | In team mode, agent status is tracked live on a single screen |
| 🔐 **Secure Auth** | API keys are stored in `~/.nina/credentials.json`, never in the repo |
...
/use <provider> [model]           → Change the active model
...
User enters a goal
   ...
   (Visible live in the terminal monitor)
        ↓
...
└── website/         # Source code for nina.efeservili.dev
```
...
cohere      → Command R+
openrouter  → 200+ models, single API
```
...
- API keys are stored only in `~/.nina/credentials.json` — never committed to the repo
- Use the `.env.example` template, never commit real values
- The `main` branch is protected — direct push is disabled
- PR review is required, critical files are protected with CODEOWNERS
...
Before opening a PR, check the related file in CODEOWNERS.
